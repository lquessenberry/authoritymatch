...
import type { GradingConfig } from "./config.js";
...
export class ScoringEngine {
  ...
  private calculateSafetyScore(authority: Authority): ScoreComponent {
    ...
    if (safety) {
      ...
      csaFields.forEach((field) => {
        const value = safety[field as keyof typeof safety] as
          | number
          | undefined;
        if (value !== undefined) {
      ...
      if (safety.crashRate !== undefined) {
        factors.crashHistory =
          safety.crashRate === 0 ? 5 : Math.max(0, 5 - safety.crashRate);
      } else {
  ...
  private calculateStabilityScore(authority: Authority): ScoreComponent {
    ...
    if (authority.addDate) {
      ...
      const now = new Date();
      const ageMonths =
        (now.getFullYear() - addDate.getFullYear()) * 12 +
        (now.getMonth() - addDate.getMonth());

      if (ageMonths < stabilityRules.minAuthorityAgeMonths) {
        // New authority penalty
        factors.yearsInBusiness = Math.max(
          0,
          50 * (ageMonths / stabilityRules.minAuthorityAgeMonths),
        );
  ...
  private calculateScaleScore(authority: Authority): ScoreComponent {
    ...
    const actualRatio =
      drivers > 0 && powerUnits > 0 ? drivers / powerUnits : 1;
    const ratioDiff = Math.abs(actualRatio - idealRatio);
    factors.powerUnitRatio = Math.max(0, 25 - ratioDiff * 10);

    score = Math.round(
      factors.fleetSize + factors.driverCount + factors.powerUnitRatio,
    );
  ...
  private calculateComplianceScore(authority: Authority): ScoreComponent {
    ...
    if (complianceRules.requireActiveInsurance) {
      factors.insuranceActive =
        authority.insurance?.status === "A"
          ? 40
          : authority.insurance?.status === "I"
            ? 10
            : 0; // Unknown
    } else {
    ...
    if (authority.mcs150Date) {
      ...
      const daysDiff = Math.floor(
        (now.getTime() - mcs150Date.getTime()) / (1000 * 60 * 60 * 24),
      );
  ...
  private calculateGeographyScore(authority: Authority): ScoreComponent {
    ...
    factors.operatingRadius =
      authority.carrierOperation === "A"
        ? 25
        : authority.carrierOperation === "C"
          ? 20
          : 15;
    ...
    score = Math.round(
      factors.stateRisk + factors.operatingRadius + factors.routeDiversity,
    );
  ...
  private scoreToGrade(score: number): Grade {
    ...
    const entries = Object.entries(gradeBoundaries) as [
      Grade,
      [number, number],
    ][];
